...
Public Function bAjouterTypeFichier(sExtension$, sTypeFichier$, _
    Optional sDescriptionExtension$ = "", _
...
Public Function bAjouterMenuContextuel(sTypeFichier$, sCmd$, _
    Optional bPrompt As Boolean = True, _
    Optional bEnlever As Boolean = False, _
    Optional sDescriptionCmd$ = "", _
    Optional sCheminExe$ = "", _
    Optional sCmdDef$ = """%1""", _
    Optional sDescriptionTypeFichier$ = "", _
    ...
    Try
        ...
        If bEnlever Then
            ...
            If bEnleverTypeFichier Then
                ...
                If bCleRegistreCRExiste(sTypeFichier) Then
                    ...
                    If bPrompt Then _
                        MsgBox("Le type de fichier [" & sTypeFichier & "]" & vbLf & _
                            "a été enlevé avec succès dans la base de registre", _
                        MsgBoxStyle.Information, m_sTitreMsg)
                Else
                    If bPrompt Then _
                        MsgBox("Le type de fichier [" & sTypeFichier & "]" & vbLf & _
                            "est introuvable dans la base de registre", _
                        MsgBoxStyle.Information, m_sTitreMsg)
                End If
            Else
                ...
                If bCleRegistreCRExiste(sCleDescriptionCmd) Then
                    ...
                    If bPrompt Then _
                        MsgBox("Le menu contextuel [" & sDescriptionCmd & "]" & vbLf & _
                            "a été enlevé avec succès dans la base de registre pour les fichiers du type :" & vbLf & _
                            "[" & sTypeFichier & "]", _
                        MsgBoxStyle.Information, m_sTitreMsg)
                Else
                    If bPrompt Then _
                        MsgBox("Le menu contextuel [" & sDescriptionCmd & "]" & vbLf & _
                            "est introuvable dans la base de registre pour les fichiers du type :" & vbLf & _
                            "[" & sTypeFichier & "]", _
                        MsgBoxStyle.Information, m_sTitreMsg)
                End If
        ...
        If bPrompt Then _
            MsgBox("Le menu contextuel [" & sDescriptionCmd & "]" & vbLf & _
                "a été ajouté avec succès dans la base de registre pour les fichiers du type :" & vbLf & _
...
Public Function bCleRegistreCRExiste(sCle$, _
    ...
    Try
        ...
        Using rkCRCle As RegistryKey = Registry.ClassesRoot.OpenSubKey( _
            sCle & "\" & sSousCle)
...
Public Function bCleRegistreCRExiste(sCle$, sSousCle$, _
...
Public Function bCleRegistreLMExiste(sCle$, _
    Optional sSousCle$ = "", _
    Optional ByRef sValSousCle$ = "", _
    ...
    Try
        ...
        ' Si la clé n'existe pas, on passe dans le Catch
        Using rkLMCle As RegistryKey = Registry.LocalMachine.OpenSubKey(sCle, _
            writable:=bEcriture)
...
Public Function bCleRegistreCUExiste(sCle$, _
    Optional sSousCle$ = "", _
